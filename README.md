...
Текущая версия: **1.0.1.4**. Лицензия: GPL-3.0-or-later.

Готовые пакеты: [tar.gz](dist/pdfMetadata-1.0.1.4.tar.gz), [ZIP](dist/pdfMetadata-1.0.1.4.zip), [SHA256](dist/SHA256SUMS.txt).
...
Для локальной обработки PDF используется один из backend:

- Poppler: `pdfinfo` + `pdftotext` — приоритетный вариант;
- Ghostscript: `gs` с устройством `txtwrite` — автоматический fallback, если Poppler недоступен;
- `prlimit` применяется к обоим backend.
...
- обработка Poppler/Ghostscript запускается без shell и с resource limits;
